...
int main(int argc, char** argv)
{
    ...
    if(Srcimg.dimX != Desimg.dimX || Srcimg.dimY != Desimg.dimY || Srcimg.numChannels!= Desimg.numChannels){
        ...
        printf("(%d,%d)\n(%d,%d)\n",Srcimg.dimX,Srcimg.dimY,Desimg.dimX,Desimg.dimY);
        exit(0);
    }
    ...
    for (int y = 0; y < Desimg.dimX* Desimg.dimY * Desimg.numChannels; y++)
    {
        ...
        num_of_untolerance += fabs(Desimg.data[y] - Srcimg.data[y]) > 1? 1 :0;
        new_delta += fabs(Desimg.data[y] - Srcimg.data[y]);
    }
    double avg_delta = delta/(Desimg.dimX* Desimg.dimY * Desimg.numChannels);
    printf("the avg_delta : %lf the num_of_untolerance : %d \ndelta: %lf\n",avg_delta,num_of_untolerance,new_delta);
    // printf("the num_of_untolerance : %d\n",num_of_untolerance);
    return 0;
}
