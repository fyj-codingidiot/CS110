...
v_add_optimized_adjacent (double *x, double *y, double *z)
{
    #pragma omp parallel
    {
        ...
        for(int i = curr_thread; i < ARRAY_SIZE; i += total_threads) {
		    z[i] = x[i] + y[i];
        }
    }
}
...
v_add_optimized_chunks (double *x, double *y, double *z)
{
     #pragma omp parallel
    {
        int total_threads = omp_get_num_threads();
        int curr_thread = omp_get_thread_num();
		    int chunk_size = ARRAY_SIZE / total_threads;
        int rest_num = ARRAY_SIZE - chunk_size * total_threads;
      ...
      if(rest_num != 0) {
			for(int i = ARRAY_SIZE - rest_num; i < ARRAY_SIZE; ++i) {
				z[i] = x[i] + y[i];
			}
		}
    }
