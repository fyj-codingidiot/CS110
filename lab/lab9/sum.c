...
int sum_vectorized( int n, int *a )
{
    ...
    int ans[4];
    int res = 0;
    _mm_storeu_si128((__m128i*)ans,sum);
    for(int i=0;i<4;i++){
        res+=ans[i];
    }
    for(int i=(n/4)*4;i<n;i++){
...
int sum_vectorized_unrolled( int n, int *a )
{
    ...
    for(int i=0;i<(n/16)*16;i+=16){ 
        __m128i a1 = _mm_loadu_si128((__m128i*)(a+i+0));
        __m128i a2 = _mm_loadu_si128((__m128i*)(a+i+4));
        __m128i a3 = _mm_loadu_si128((__m128i*)(a+i+8));
        __m128i a4 = _mm_loadu_si128((__m128i*)(a+i+12));
        sum = _mm_add_epi32(a1,sum);
